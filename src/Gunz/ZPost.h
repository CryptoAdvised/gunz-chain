...
auto lastZPostSkill = chrono::high_resolution_clock::now(); 
auto lastZPostShot = chrono::high_resolution_clock::now(); 
auto lastZPostShotMelee = chrono::high_resolution_clock::now(); 
...
{
	//Anti Flipmower
	auto liveStemp = chrono::high_resolution_clock::now();    
    auto dur = liveStemp - lastZPostSkill; 
	auto zPostDelay = (float)std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
	lastZPostSkill = chrono::high_resolution_clock::now(); 
	if(zPostDelay < 100.0f)return;
...
inline void ZPostShot(float fShotTime, const rvector &pos, const rvector &to,int sel_type)
{
	//Anti 0delay
	auto liveStemp = chrono::high_resolution_clock::now();    
    auto dur = liveStemp - lastZPostShot; 
	auto zPostDelay = (float)std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
	lastZPostShot = chrono::high_resolution_clock::now(); 
	if(zPostDelay < 10.0)return;
...
inline void ZPostShotMelee(float fShotTime, rvector &pos, int nShot)
{
	//Anti Lawnmower
	auto liveStemp = chrono::high_resolution_clock::now();    
    auto dur = liveStemp - lastZPostShotMelee; 
	auto zPostDelay = (float)std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
	lastZPostShotMelee = chrono::high_resolution_clock::now(); 
	if(zPostDelay < 100.0f)return;
...
inline void ZPostRGSlash(const rvector &Pos, const rvector &Dir, int Type)
{
	//Anti Lawnmower
	auto liveStemp = chrono::high_resolution_clock::now();    
    auto dur = liveStemp - lastZPostRGSlash; 
	auto zPostDelay = (float)std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
	lastZPostRGSlash = chrono::high_resolution_clock::now();
	if(zPostDelay < 100.0f)return;
...
{
	//Anti Insane Massive / Flipmower
	auto liveStemp = chrono::high_resolution_clock::now();    
    auto dur = liveStemp - lastZPostRGMassive; 
	auto zPostDelay = (float)std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
	lastZPostRGMassive = chrono::high_resolution_clock::now(); 
	if(zPostDelay < 100.0f)return;
