...
bool GetItemDescStr(string& str, DWORD nItemID) {
	...
	if (pItemDesc->m_nResLevel) {
		if (bAdd) str += " / ";
		sprintf_safe(temp, "���ѷ���:%d", pItemDesc->m_nResLevel);
	...
	if (pItemDesc->m_nWeight) {
		if (bAdd) str += " / ";
		sprintf_safe(temp, "����:%d", pItemDesc->m_nWeight);
	...
	if (pItemDesc->m_nMaxBullet) {
		if (bAdd) str += " / ";
		sprintf_safe(temp, "�ִ�ź�� : %d", pItemDesc->m_nMaxBullet);
	...
	if (pItemDesc->m_nMagazine) {
		if (bAdd) str += " / ";
		sprintf_safe(temp, "źâ : %d", pItemDesc->m_nMagazine);
	...
	if (pItemDesc->m_nDamage) {
		if (bAdd) str += " / ";
		sprintf_safe(temp, "���ݷ� : %d", pItemDesc->m_nDamage);
	...
	if (pItemDesc->m_nDelay) {
		if (bAdd) str += " / ";
		sprintf_safe(temp, "������ : %d", pItemDesc->m_nDelay);
	...
	if (pItemDesc->m_nReloadTime) {
		if (bAdd) str += " / ";
		sprintf_safe(temp, "�����ð� : %d", pItemDesc->m_nReloadTime);
	...
	if (pItemDesc->m_nMaxWT) {
		if (bAdd) str += " / ";
		sprintf_safe(temp, "+�ִ빫�� : %d", pItemDesc->m_nMaxWT);
